...
program.requiredOption('-f, --file <path-to-file>', 'path to file')
.option('-id, --id <id>', 'id in the url e.g. http://localhost:8080/:id')
.option('-p, --port <port>', 'port to listen on. If port is taken then program will try to find the next available port', (val, _) => parseInt(val), 8000)
.parse(process.argv);
...
const listener = (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (req.url != null && req.url === `/${program.id}`) {
        ...
        let readStream = createReadStream(program.file);
        readStream.pipe(res)
    } else {
        res.end()
    }
